...
public class PoseReceiver_NoUI : MonoBehaviour
{
    private PoseLandmarkerRunner_NoUI runner;

    private void Start()
    {
        runner = PoseLandmarkerRunner_NoUI.Instance;

        if (runner == null)
        {
            Debug.LogError("PoseReceiver 找不到 PoseLandmarkerRunner_NoUI！");
            return;
        }
